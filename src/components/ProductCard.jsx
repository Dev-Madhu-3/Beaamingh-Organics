...
const ProductCard = ({ product }) => {
  ...
  const handleAddToCart = () => {
    ...
    window.open(
      `https://wa.me/${mobileNumber}?text=${encodeURIComponent("Hey, I would like to order the product " + product.name + " priced at ₹ " + product.price)}`,
      '_blank'
    );
  };
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 transform hover:-translate-y-1">
        <div className="relative">
          <img
            src={
              product.image ||
              "https://res.cloudinary.com/dpk6qsn0e/image/upload/v1758452286/sorry-image-not-available_yyzkrp.jpg"
            }
            alt={product.name}
            className={`m-auto h-48 ${product.image && "object-cover w-full"}`}
          />
